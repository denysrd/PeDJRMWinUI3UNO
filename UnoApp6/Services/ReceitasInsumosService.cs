...
public class ReceitasInsumosService
{
    ...
    public async Task<bool> RemoverPorVersaoReceitaIdAsync(int versaoReceitaId)
    {
        if (versaoReceitaId <= 0)
            throw new ArgumentException("ID da versão inválido.");

        return await _receitasInsumosRepository.DeleteByVersaoReceitaIdAsync(versaoReceitaId);
    }

    public async Task<bool> RemoverPorVersaoReceitaAsync(int versaoReceitaId)
    {
        if (versaoReceitaId <= 0)
            throw new ArgumentException("ID da versão da receita inválido.");
